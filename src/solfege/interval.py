...
class ChromaticInterval(_Interval):
    ...
    def __add__(self, other):
        if not isinstance(other, ChromaticInterval):
            raise Exception(
                "Adding a ChromaticInterval interval to something which is not a ChromaticInterval but %s" % other)
        return super().__add__(other)
    ...
    def getDiatonic(self):
        ...
        if "diatonic" not in self.dic:
            diatonic = self.RelatedDiatonicClass(diatonic=[0, 0, 1, 2, 2, 3, 3, 4, 5, 5, 6, 6][
                                                              self.getSameNoteInBaseOctave().getNumber()] + 7 * self.getOctave())
...
class SolfegeInterval(ChromaticInterval):
    ...
    def addOctave(self, nb):
        Class = self.ClassToTransposeTo or self.__class__
        return Class(chromatic=self.getChromatic().addOctave(nb).getNumber(),
                     diatonic=self.getDiatonic().addOctave(nb).getNumber())
