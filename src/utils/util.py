...
from typing import Any, Callable, Dict, Iterable, List, Optional, Self, Sized, Type, TypeVar
import unittest
import traceback
...
def debug(string: str, params: Optional[Any] = None) -> None:
...
def list2dic(l: List[T]) -> Dict[int, T]:
...
class MyException(Exception):
    ...
    dic: Dict[Any, Any]
    """The attached `key: value` debugging context, shown in `str(self)`."""

    def __init__(self) -> None:
        """Start with no attached information."""
        self.dic = dict()

    def addInformation(self, key: Any, value: Any) -> None:
...
def indent(str: str, nb_space: int = 2) -> str:
...
def tests_modules(modules: List[ModuleType]) -> None:
...
def assert_all_same_class(it: Iterable) -> Optional[bool]:
...
def assert_string_equal(string1: str, string2: str) -> None:
...
def assert_typing(value: Any, type: Type, exact:bool=False) -> None:
...
def assert_iterable_typing(it: Iterable, type: Type, exact:bool = False) -> None:
...
def assert_optional_typing(value: Any, type: Type, exact:bool = False) -> None:
...
def sorted_unique(it: Iterable[T]) -> List[T]:
...
def img_optional_tag(filename: Optional[str]) -> str:
...
def assert_equal_length(l: List[Sized]) -> None:
