...
from fretted_instrument.chord.open.chromatic_note_list_to_guitar_chords import ChromaticNoteListToGuitarChords
...
def generate_anki_notes():
    for note_list, chromatic_note_and_its_guitar_chord in note_to_chord:
        ...
        for guitar_chord in chromatic_note_and_its_guitar_chord.maximals():
            ...
            save_file(f"{open_folder}/{guitar_chord.file_name(stroke_colored=False, absolute=True)}", guitar_chord.svg(absolute=True))
            save_file(f"{open_folder}/{guitar_chord.file_name(stroke_colored=True, absolute=True)}", guitar_chord.svg(absolute=True, colors=ChordColors(tonic)))
generate_anki_notes()
