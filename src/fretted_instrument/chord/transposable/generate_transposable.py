from fretted_instrument.chord.guitar_chord import ChordColors
from fretted_instrument.chord.transposable.chromatic_interval_list_to_guitar_chords import ChromaticIntervalListToGuitarChords
from fretted_instrument.chord.chord_utils import enumerate_guitar_chords
...
def generate_anki_notes():
    for interval_list, chromatic_interval_and_its_guitar_chord in interval_to_chord:
        ...
        for guitar_chord in chromatic_interval_and_its_guitar_chord.maximals():
            ...
            save_file(f"{transposable_folder}/{guitar_chord.file_name(stroke_colored=False, absolute=False)}", guitar_chord.svg(absolute=False))
            save_file(f"{transposable_folder}/{guitar_chord.file_name(stroke_colored=True, absolute=False)}", guitar_chord.svg(absolute=False, colors=ChordColors(tonic)))
generate_anki_notes()
